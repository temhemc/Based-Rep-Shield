...
export default function Home() {
  ...
  // GÖRSELDEKİ GERÇEK ADRESİNİ BURAYA YAZDIM
  ...
  useEffect(() => {
    const analyzeWallet = async () => {
      ...
      // MASTER BUILDER MODU - HİÇBİR API BEKLEMEZ, DİREKT BASAR
      if (isUserMaster) {
        setTimeout(() => {
          setStats({
            score: 99,
      ...
      try {
        ...
        if (data.status === "1" && Array.isArray(data.result)) {
          ...
          setStats(s => ({ ...s, score: 15, txCount: 1 }));
        }
      } catch (e) {
        setStats(s => ({ ...s, score: 10, txCount: 0 }));
      } finally {
        setLoading(false);
      }
    };
          ...
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
            {loading ? (
              <div className="py-20 flex flex-col items-center gap-4">
                ...
                <p className="text-white/40 font-bold animate-pulse text-[10px] uppercase tracking-widest">Scanning Base Mainnet...</p>
              </div>
            ) : (
              <>
                <div className="bg-gradient-to-br from-white/15 to-transparent p-6 rounded-[2.5rem] border border-white/10 flex justify-between items-center relative overflow-hidden">
                ...
                <div className="bg-white/5 border border-white/10 p-4 rounded-2xl flex items-center justify-between">
                  <div className="flex items-center gap-2 text-white/40">
                    <Coins size={16} />
                    <span className="text-[10px] font-black uppercase tracking-tighter">Balance</span>
                  </div>
